...
public class TripPoint {
	...
	public static int h1StopDetection() {
		int stops = 0; // stop counter
	    movingTrip = new ArrayList<>();
	    movingTrip.add(trip.get(0));
	    int i = 1;
	    while (i < trip.size()) {
	        if (haversineDistance(trip.get(i), trip.get(i - 1)) >= 0.6) // displacement threshold of 0.6 km
	        {
	...
	//find and return the amount of time spent moving in hours
	...
	//find and return the amount of time spent stopped in hours
    public static double stoppedTime() {
    	 double movingT = movingTime();
 	    double totalT = totalTime();
 	    return totalT - movingT;
    }

    //find and return the average speed over the course of the trip only considering the moving points. Units should be km/hour
    public static double avgMovingSpeed(){
    	double dist = 0;
	    int i = 1;
	    while (i < movingTrip.size()) {
